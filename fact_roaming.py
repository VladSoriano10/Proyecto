...
def cargar_fact_roaming():
    ...
    # Para la tasa de cambio, necesitamos la llave y la fecha desde donde inicia su vigencia
    df_dim_tasa = pd.read_sql("SELECT sk_tasa, fecha_desde FROM dim_tasa_cambio;", con=engine_destino)


    # TRANSFORMACIÓN Y CRUZAMIENTO

    print("-> 3. Ejecutando cruces masivos y asignando llaves (-1)...")
    ...
    # --- E. Lookup Temporal (Tasa de Cambio) ---
    ...
    # merge_asof busca hacia atrás la tasa que estaba vigente en el momento exacto de la transacción
